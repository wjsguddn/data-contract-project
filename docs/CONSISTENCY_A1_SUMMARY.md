...
- **입력**: 사용자 계약서 조항 (제목 + 본문)
- **처리**: 
  - 조항 임베딩 생성 (제목 + 본문 가중 평균)
  - FAISS 벡터 검색 (의미 유사도)
  - Whoosh 키워드 검색 (BM25)
  - 하이브리드 점수 계산 (Dense 85% + Sparse 15%)
- **출력**: Top-K 후보 표준 조항 및 점수
...
- **입력**: 사용자 조항 + Top-3 후보 표준 조항
- **처리**: GPT-4가 의미적 일치 여부 판단
  - 내용 비교 (목적, 범위, 조건 등)
  - 법적 의미 동등성 확인
  - 표현 차이 vs 실질 차이 구분
- **출력**: 
...
[모든 조항 처리 완료]
    ↓
4단계: article_mapping.json 저장
    - matched: 양방향 매칭된 조항 쌍
    - missing: 누락된 표준 조항
    - extra: 추가된 사용자 조항
    ↓
5단계: A2/A3 Node 트리거 (병렬)
