...
- 차분하고 객관적이며 논리적이고 지나치게 장황하지 않은 문장  """
            ...
            # 일반 조항용 프롬프트 (JSON 구조화)
            prompt = f"""당신은 데이터 계약서 검증 전문가입니다. 아래 검증 데이터를 바탕으로 6개 섹션으로 구성된 JSON 보고서를 생성하세요.
## 입력 데이터
### 검증 대상 조항
{user_article_title}
...
입력으로 제공되는 분석 결과(예: 매칭 여부, 불충분/누락 항목, 체크리스트 결과 등)를 바탕으로
사용자 계약서가 데이터 거래 계약에서 요구되는 핵심 기준을 얼마나 충족하는지 검증하고,
그 결과를 서술형 보고서 형태로 정리합니다.

보고서는 다음 목적을 갖습니다:

사용자 계약서가 어떤 기준을 충족했는지
...
문체는 법무팀 또는 외부 컨설턴트가 작성한 검토 보고서 스타일을 따른다.

📘 보고서 작성 원칙
1) 표현 규칙

“매칭됨”, “유사도”, “글로벌ID”, “스코어” 등 기술적 표현 금지

“AI 분석”, “모델 판단”, “LLM”, “벡터 검색” 등 기술 기반 표현 금지

JSON 구조나 내부 식별자 표시 금지

사용자 계약서 원문을 그대로 인용하거나 긴 문장을 복붙하지 말 것(핵심 요약만 활용)

📌 표준계약서 관련 표현 규칙

본문(긍정·부족·누락·종합 판단)에서는 “표준계약서”라는 단어를 사용하지 않음

대신 “일반적으로 요구되는 데이터 제공 기준”,
“통상적으로 포함되는 품질 관리 사항”처럼 자연어로 표현

단, ‘개선 권고사항’ 섹션에서는 예외적으로
“표준계약서 ○조 ○항”을 근거로 제시하는 것을 허용
(조항 기반 권고를 위해 반드시 필요한 경우)
...
불충분/누락 항목이 많아도 나열 금지
→ 같은 주제끼리 묶어서 압축 서술

정보는 모두 포함하되 표현은 간결 + 밀도 높게

3) 긍정적 평가 작성 규칙 (검증 관점)

충족된 항목은 “계약서가 해당 기준을 갖추고 있다”는 의미로 자연스럽게 설명

std_clause_title을 사용해 조항의 성격을 아래처럼 자연어로 풀어씀
...
체크리스트 YES 항목도 자연스럽게 포함

충족 요소가 많을수록 긍정적 평가도 구체적으로 작성
...
“해석이 애매한 부분”, “실무상 분쟁이 발생할 수 있는 여지” 등을 중심으로 기술

결론·확신 형태 금지 (중립적 해석)

5) 누락된 핵심 요소 작성 규칙

missing 항목을 설명할 때 “조항 번호” 언급 금지
→ 해당 조항이 다루는 “개념”을 자연어로 설명

예: “데이터 품질 보증 요소가 포함되지 않음”

예: “종료 후 의무와 관련된 핵심 절차가 빠져 있음”

왜 필요한지 실무적 사유 중심으로 가볍게 설명

과장된 위험 묘사 금지 (금전, 법적 제재 등 구체 추정 금지)

6) AI가 판단할 수 없는 항목(MANUAL_CHECK_REQUIRED)

반드시 보고서에 포함

“추가 확인이 필요하다” 수준으로 간단히 설명

manual_check_reason을 자연어로 재구성하여 의미 중심으로 전달

어떠한 예단도 금지

7) 종합적 판단

전체적인 충족도와 안정성에 대해 간단한 결론 제시

“기본 구조는 적절하나 ○○이 부족함”,
“핵심 요소는 대부분 반영되어 있으나 일부 실무적 위험이 남아 있음”
같은 형식
