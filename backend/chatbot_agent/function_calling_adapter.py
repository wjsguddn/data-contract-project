...
1. **get_article_by_index**: 조 번호 또는 별지 번호로 해당 항목 내용 직접 조회
   ...
   - 사용자 계약서 미탐색 항목 중 질문과 연관된 정보가 있을 것으로 판단되는 항목들이 있다면 사용합니다.
     예시: 사용자가 계약의 만료에 대한 질문을 했고, 미탐색 항목 중 "계약 기간"조항이 의미상 동일하므로 선택 조회 등
   - 수집된 사용자 계약서의 내용에 특정 조나 별지에 대한 참조가 명시되어있고(ex. 수집한 사용자 계약서 조항의 내용에서 "~는 별지 2에 기재된 바에 따르며"와 같이 다른 조항을 명시하는 등의 경우), 해당 참조의 내용이 필요하다고 판단되면 사용합니다.
...
3. **hybrid_search**: 쿼리 기반 하이브리드 검색
   사용 시기:
   - 미탐색 항목중에 질문의 주제와 확실하게 연관되어 보이는 항목이 없다면 사용합니다.
...
4. **lookup_standard_contract**: 표준계약서 조문 조회 (참고용 템플릿)
   사용 시기:
   - 사용자가 계약서 내용의 작성이나 형식 검증에 대한 질문을 한 경우 사용합니다.
   - 사용자 계약서의 조항 중에서 모범 템플릿 형식을 파악하고 싶은 조항의 번호와 해당 조항 관련 주제를 입력합니다.
   - 조항 번호를 특정할 수 없는 경우, 주제만 입력합니다. (ex. 질문:"데이터 제공 방식에 대해 명시하는 내용을 작성하고 싶어"이며 사용자 계약서에 데이터 제공 방식 관련 조항이 존재하지 않는 경우)
...
- 제목 언급 → get_article_by_title
- 내용 기반 조회 필요 → hybrid_search
- 작성/형식의 올바름 질문 → lookup_standard_contract

중요:
- 이미 실행한 도구는 절대로 같은 파라미터로 다시 호출하지 마세요.
- 같은 도구를 다른 파라미터로는 호출 가능합니다. 단 모든 파라미터가 사용한 적 없는 값이어야 합니다.
- 필요에 따라 여러 도구를 동시에 선택해도 좋습니다.
- 사용자 계약서의 특약 조항에는 어떤 내용이 작성되어 있을지 모릅니다. 찾고 있는 내용이 일반 조항에서 명확히 발견되지 않았다면, 특약사항 조항을 확인하는 것이 좋습니다.
"""
        return prompt
