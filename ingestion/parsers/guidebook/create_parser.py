...
PAGE_START = 74
PAGE_END   = 89
OUT_JSON = Path(r"C:\Users\USER\Downloads\pdf-ocr-test - 복사본\markitdowntext\parsed_74_89_table_1.json")
# ========================
...
def parse_md_page(md_text: str,
    ...
    for raw in lines:
        ...
        # ✅ 법령 인용 감지 (문장 전체가 법령 안내일 때만 새 해설 섹션으로 분리)
        if re.search(r"「[^」]{3,}법」", line):
            # ① 조문 섹션이면 무조건 본문에 붙임
            if open_buf.get("type") == "조":
                open_buf["content_lines"].append(line)
                continue

            # ② 해설인데, 문장 길이가 길거나 일반 설명문처럼 보이면 그대로 붙임
            if open_buf.get("type") == "해설":
                # 문장 중간 인용이라면 분리하지 않음 (일반적으로 2문장 이상 or 쉼표 포함)
                if len(line.strip()) > 25 or " " in line.strip().replace("「", "").replace("」", ""):
                    open_buf["content_lines"].append(line)
                    continue

            # ③ 정말 독립된 법령 안내 문단인 경우만 새 해설로 분리
            if re.fullmatch(r"\s*(>?\s*「[^」]+법」.*)", line.strip()):
                title_for_expl = (open_buf.get("title") if open_buf else carry_title)
                subtitle = "관련 법령 해설"
                start_new_section("해설", title_for_expl, subtitle, True)
                open_buf["content_lines"].append(line)
                continue
